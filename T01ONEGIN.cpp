 ...
 */
int CheckSort(type_data *lines, size_t size, int(*Compare)(const type_data *, const type_data *));
 ...
 * \param[out] none.
 */
void SkipTrash(mk5::line *line, const char *mode)
{
  if (mode == "end")
...
int StrCompare(const type_data *strA, const type_data *strB)
{
  ...
  SkipTrash(&str1, "begin");
  SkipTrash(&str2, "begin");
...
int ReadStrBuf(mk5::line **lines, const char *FileName, mk5::line *buf)
{
  ...
  int size_of_file = 0, count_of_strings = 0;
  ...
  buf->str = (char*) calloc(size_of_file, sizeof(char));
  buf->len = size_of_file;
  fread(buf->str, sizeof(char), buf->len - 1, file);
  fclose(file);

  buf->str[buf->len - 1] = 0;
  
  //TODO Осмысленные названия переменным
  int i_sym = 0;
  while (buf->str[i_sym++] != 0)
    if (buf->str[i_sym] == '\n')
      count_of_strings++;
  count_of_strings++;
...
int CheckSort(type_data* lines, size_t size, int(*Compare)(const type_data*, const type_data*))
{
  for (size_t i_line = 1; i_line < size ; i_line++)
    if (Compare(lines + i_line - 1, lines + i_line) > 0)
      return 0;
